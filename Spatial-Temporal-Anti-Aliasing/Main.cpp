                     ...
                     int       nCmdShow)
{
	...
	int aliasing = 0;
	sscanf(lpCmdLine, "%d %f %f %f %d", &numOfLine, &speedOfRotation, &fps, &scale, &aliasing);
	isAntiAliasing = (aliasing == 1);
	if (isAntiAliasing && fps < speedOfRotation * 2) {
		speedOfRotation = fps / 2 + 0.01;
	}
	...
	printf("The second parameter was : %f\n", speedOfRotation);
	printf("The third parameter was : %f\n", fps);
	printf("The fourth parameter was : %f\n", scale);
	printf("The fifth parameter was : %f\n", aliasing);
...
void AntiAliasing(char* imgData, int width, int height, float scale) {
	for (int i = 0; (i * scale) < height; i++) {
		...
		for (int j = 0; (j * scale) < width; j++) {
			...
			for (int yy = max(0, y - 1); yy <= min(y + 1, height - 1); yy++) {
				for (int xx = max(0, x - 1); xx <= min(x + 1, width - 1); xx++) {
					b += imgData[(xx * 4) + (yy * width * 4) + 0];
					g += imgData[(xx * 4) + (yy * width * 4) + 1];
					r += imgData[(xx * 4) + (yy * width * 4) + 2];
					a += imgData[(xx * 4) + (yy * width * 4) + 3];
					cnt++;
				}
			}

			imgData[(x * 4) + (y * width * 4) + 0] = (byte)(b / cnt);
			imgData[(x * 4) + (y * width * 4) + 1] = (byte)(g / cnt);
			imgData[(x * 4) + (y * width * 4) + 2] = (byte)(r / cnt);
			imgData[(x * 4) + (y * width * 4) + 3] = (byte)(a / cnt);
		}
	}
}
...
void ScaleImage(char* imgData, int width, int height, char* destData, float scale) {
	...
	for (int i = 0; i < scaledHeight; i++) {
		...
		for (int j = 0; j < scaledWidth; j++) {
			int x = j * scale;
			destData[(j * 4) + (i * scaledWidth * 4) + 0] = imgData[(x * 4) + (y * height * 4) + 0];
			destData[(j * 4) + (i * scaledWidth * 4) + 1] = imgData[(x * 4) + (y * height * 4) + 1];
			destData[(j * 4) + (i * scaledWidth * 4) + 2] = imgData[(x * 4) + (y * height * 4) + 2];
			destData[(j * 4) + (i * scaledWidth * 4) + 3] = imgData[(x * 4) + (y * height * 4) + 3];
		}
	}
}
